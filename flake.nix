{
  description = "hetzner_auction_bot";
  ...
  outputs = {
      ...
      forAllSystems = function:
        nixpkgs.lib.genAttrs [
          ...
          "aarch64-darwin"
            ...
            hetzner_auction_bot = craneLib.buildPackage (craneCommon
              // {
    ...
    in {
      checks = forAllSystems ({
        ...
        inherit hetzner_auction_bot;

        hetzner_auction_bot-clippy = craneLib.cargoClippy (craneCommon
          // {
            inherit cargoArtifacts;
        ...
        # Check formatting
        hetzner_auction_bot-fmt = craneLib.cargoFmt {
          inherit src;
        };

        # Audit dependencies
        hetzner_auction_bot-audit = craneLib.cargoAudit {
      ...
      packages = forAllSystems ({ hetzner_auction_bot, ... }: {
        inherit hetzner_auction_bot;
        default = hetzner_auction_bot;
      });

      apps = forAllSystems ({system, ...}: {
        hetzner_auction_bot = {
          type = "app";
          program = "${self.packages.${system}.hetzner_auction_bot}/bin/hetzner_auction_bot";
        };
        default = self.apps.${system}.hetzner_auction_bot;
      });
      ...
      devShells = forAllSystems ({
        ...
        devshell = {
          name = "hetzner_auction_bot-devshell";
          packages = with pkgs; [
            # Rust build inputs
            clang
            coreutils
